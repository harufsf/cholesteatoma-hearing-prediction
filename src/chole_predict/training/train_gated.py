...
from chole_predict.training.gated_engine import eval_gated, train_gated_epoch
...
def _fit_gated_model(
    ...
    for _ in range(epochs):
        ...
        va_loss, *_ = eval_gated(model, va_ld, device, use_amp=use_amp, gate_use_ytab=gate_use_ytab)
        if va_loss < best_val:
            best_val = va_loss
            best_state = {k: v.detach().cpu().clone() for k, v in model.state_dict().items()}

    if best_state is None:
        raise RuntimeError("Gated training failed to produce a checkpoint.")
    model.load_state_dict(best_state)
    return model, best_val


def build_parser() -> argparse.ArgumentParser:
    ap = argparse.ArgumentParser(description="Train gated residual ROI-on-tabular model with fixed folds.")
    ...
    ap.add_argument("--roi_pool", default="concat", choices=["concat", "mean", "attn"])
    ...
    ap.add_argument("--gate_use_ytab", action="store_true")
    ap.add_argument("--lambda_gate_l1", type=float, default=1e-3)
    ap.add_argument("--lambda_delta_l2", type=float, default=1e-4)
    ap.add_argument("--val_size", type=float, default=0.2)
    ap.add_argument("--seed", type=int, default=42)
    ap.add_argument("--start_fold", type=int, default=1)
    ap.add_argument("--num_workers", type=int, default=0)
    ap.add_argument("--no_amp", action="store_true")
    ap.add_argument("--out_prefix", default="GATED_RESID_ROI_on_TAB_fixedfold")
    return ap
...
def run_train_gated(
    ...
    num_workers: int = 0,
    no_amp: bool = False,
    out_prefix: str = "GATED_RESID_ROI_on_TAB_fixedfold",
) -> None:
    if csv is None or target is None or roi_cols is None:
        args = build_parser().parse_args()
        return run_train_gated(
            csv=args.csv,
            target=args.target,
            ...
            roi_lr=args.roi_lr,
            gate_use_ytab=args.gate_use_ytab,
            lambda_gate_l1=args.lambda_gate_l1,
            lambda_delta_l2=args.lambda_delta_l2,
    ...
    roi_pool_model = "concat" if roi_pool == "attn" else roi_pool
    ...
    for fold in folds:
        ...
        gated_model, best_gated_val = _fit_gated_model(
            tr_df=tr_roi_df,
            va_df=va_roi_df,
            spec=spec,
            scaler=scaler,
        ...
        _, ids, y_true, y_tab, y_hat, delta, gate = eval_gated(gated_model, te_ld, device, use_amp=use_amp, gate_use_ytab=gate_use_ytab)
        metrics = evaluate_metrics(y_true, y_hat, target_cols)
        ...
        for k, t in enumerate(target_cols):
            ...
            oof[f"pred_{t}"] = y_hat[:, k]
            oof[f"y_tab_{t}"] = y_tab[:, k]
            oof[f"delta_{t}"] = delta[:, k]
            oof[f"gate_{t}"] = gate[:, k]
            oof[f"err_{t}"] = y_hat[:, k] - y_true[:, k]
            oof[f"abs_err_{t}"] = np.abs(y_hat[:, k] - y_true[:, k])
        oof_rows.append(oof)
        ...
        summary_rows.append({
            ...
            "best_gated_val_loss": best_gated_val,
            "gate_use_ytab": gate_use_ytab,
            "lambda_gate_l1": lambda_gate_l1,
            "lambda_delta_l2": lambda_delta_l2,
        })
        ...
        torch.save(gated_ckpt, f"{out_prefix}_fold{fold}_gated.pt")
...
def load_gated_checkpoint(ckpt_path: str | Path, device: str | torch.device = "cpu"):
    ...
    model = ResidualGatedROIModel(
        ...
        n_rois=len(ckpt["roi_cols"]),
        roi_pool=ckpt.get("roi_pool", "concat"),
        gate_use_ytab=bool(ckpt.get("gate_use_ytab", False)),
    )
...
__all__ = ["build_parser", "run_train_gated", "load_gated_checkpoint"]
